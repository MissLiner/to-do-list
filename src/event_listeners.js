...
import { makeThisDropdown } from "make-this/src/dropdown";
...
function loadBaseListeners() {
  ...
  makeThisDropdown(editBtn, editMenu);
  makeThisDropdown(sortBtn, sortMenu);
  ...
  function controlMenuBar(event) {
    switch (event.target) {
      ...
      case helpBtn:
        alert(
          "Help you?!? I'm barely keeping my own shit together. Sorry buddy!"
        );
        break;
    }
  }
  ...
  submitItemBtn.addEventListener("click", (e) => {
    ...
    currentList == projects
      ? createDropdown(projects, projectSelect.id)
      : createDropdown(categories, categorySelect.id);
    displayTasks();
...
export { loadBaseListeners, loadTaskListeners };
